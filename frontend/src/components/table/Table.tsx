...
const TableContent = memo(
  ({
    ...
    const filteredData = useMemo(() => {
      ...
      if (postSearch.trim()) {
        filtered = filtered.filter((user) => (user.posts || []).length > 0);
      }
    ...
    return (
      <div className="overflow-x-auto">
        <table
          className="min-w-full border-collapse border-2 border-gray-300 bg-white"
            ...
            {table.getRowModel().rows.map((row) => (
              ...
              >
                {row.getVisibleCells().map((cell) => (
                  <td
                    key={cell.id}
                    className="border border-gray-300 px-4 py-2 relative bg-white hover:bg-blue-50 transition-colors"
                    style={{ overflow: "visible" }}
                  >
